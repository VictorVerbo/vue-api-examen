    ...
    <div v-else class="pokemon-container">
      <div v-for="pokemon in visiblePokemon" :key="pokemon.id" class="pokemon-card" @click="toggleDetails(pokemon)">
        ...
        <div class="pokemon-info">
          ...
          <!--Detalles de pokemon al dar click-->
...
export default {
  ...
  methods: {
    //obtención de pokemons consumiendo la api
    fetchPokemonData() {
      this.loading = true;
      axios.get(`https://pokeapi.co/api/v2/pokemon?limit=${this.limit}&offset=${this.offset}`)
        ...
        .catch(error => {
          alert('Error al obtener información de los Pokémon:', error);
          console.error('Error al obtener información de los Pokémon:', error);
          this.loading = false;
        });
    },
    //Obetención de detalles del pokemon
    ...
    //Busqueda del pokemon por id o nombre
    searchPokemon() {
      if (this.searchQuery.trim() === '') {
        ...
        axios.get(`https://pokeapi.co/api/v2/pokemon/${searchTerm}`)
          .then(response => {
            ...
            if (!imageUrl) {
              // Si no hay una imagen disponible, utilizar una imagen de error predeterminada
              alert("¡Aun no existe una imagen para este Pokémon!")
              imageUrl = require("./assets/error.png")
            }
          ...
          .catch(error => {
            alert('Error al obtener información del Pokémon:', error);
...
/* Estilo información de tarjetas*/
.pokemon-info {
  text-align: center;
...
/* Estilo para los nombre Pokémon */
.pokemon-name {
...
/*Espaciado de los botones de paginación */
.btn {
  margin-left: 10px;
  margin-right: 10px;
}
/* Creación de pokeball y animación*/
.pokeball {
  width: 10rem;
  height: 10rem;
